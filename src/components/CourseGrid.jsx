...
const categoryIcons = {
  CYBERSECURITY: <ShieldAlert className="w-5 h-5 text-emerald-500" />,
  PREDICTIVE_MODELING_ML: <Cpu className="w-5 h-5 text-purple-500" />,
  NETWORKING: <Network className="w-5 h-5 text-cyan-500" />,
  WEB_DEVELOPMENT: <Globe className="w-5 h-5 text-amber-500" />,
};
    ...
    <section className="py-20 bg-[#05070a] border-b border-slate-900 transition-colors" id="courses">
      <div className="max-w-7xl mx-auto px-6">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between mb-12 gap-4">
          ...
          <div className="font-mono text-xs text-slate-700 dark:text-slate-400 bg-slate-100 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 px-4 py-2 rounded-xl self-start md:self-auto flex items-center gap-2 shadow-sm">
            <span>Identity:</span>
            ...
            return (
              ...
              >
                <div>
                  ...
                  <h3 className="text-xl font-bold text-slate-900 dark:text-white group-hover:text-emerald-600 dark:group-hover:text-cyan-300 transition mb-3">
                    {course.title}
                  </h3>
                  ...
                  <div className="flex flex-wrap gap-1.5 mb-6">
                    {course.skills?.map((skill, idx) => (
                      <span
                        key={idx}
                        className="px-2.5 py-1 rounded-lg bg-slate-100 dark:bg-slate-900/90 text-slate-700 dark:text-slate-300 text-[11px] font-mono border border-slate-200 dark:border-slate-800 font-medium"
                      >
                        {skill}
                      </span>
                    ))}
                ...
                <div className="pt-4 border-t border-slate-200 dark:border-slate-800/80 flex items-center justify-between">
                  ...
                  <Link
                    ...
                    className="inline-flex items-center space-x-1.5 px-4 py-2 rounded-xl bg-slate-900 hover:bg-emerald-600 dark:bg-slate-800 dark:hover:bg-emerald-500 text-white dark:text-slate-200 dark:hover:text-slate-950 text-xs font-bold font-mono transition border border-slate-800 dark:border-slate-700 hover:border-emerald-500 shadow-sm"
                  >
