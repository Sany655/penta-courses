    ...
    <main className="max-w-5xl mx-auto px-4 py-20 font-sans">
      ...
      {loading ? (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {[1, 2, 3, 4].map(n => (
            <div key={n} className="p-6 rounded-2xl border border-slate-800 bg-slate-900/40 animate-pulse h-64 flex flex-col justify-between">
        ...
        <div className="p-16 text-center border border-dashed border-slate-800 rounded-2xl bg-slate-900/30 text-slate-400 font-mono text-sm">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {courses.map(course => {
            ...
            return (
              <div
                key={course.id}
                className="p-6 rounded-2xl border border-slate-800 bg-slate-900/60 hover:border-emerald-500/40 transition-all flex flex-col justify-between group shadow-xl"
              >
                <div>
                  <div className="flex items-start justify-between mb-4">
                    <span className="px-2.5 py-1 rounded bg-emerald-500/10 text-emerald-400 font-bold text-[10px] font-mono uppercase tracking-wider">
                ...
                <div className="space-y-4 pt-4 border-t border-slate-800/80">
                  <div className="flex items-center justify-between text-xs text-slate-400 font-mono">
                    <div className="flex items-center gap-3">
                      <span className="flex items-center gap-1"><BookOpen className="w-3.5 h-3.5 text-cyan-400" /> {modulesCount} Phases</span>
                      <span>•</span>
                      <span className="text-cyan-400 flex items-center gap-1"><Award className="w-3.5 h-3.5" /> Verified Cert</span>
                    </div>
                  ...
                  <div className="flex items-center gap-3 pt-2">
                    ...
                    <Link
                      ...
                      className="flex-1 py-2.5 rounded-xl bg-emerald-500 hover:bg-emerald-400 text-slate-950 text-xs font-bold text-center transition flex items-center justify-center gap-1 shadow-lg shadow-emerald-500/20"
                    >
