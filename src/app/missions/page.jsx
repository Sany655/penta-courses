...
export default function AdaptiveMissionsPage() {
  ...
  const startNewMission = async () => {
    ...
    try {
      ...
      // 2. Fetch adaptive mission
      if (currentSession?.id) {
        const missionRes = await fetch(`/api/v1/sessions/${currentSession.id}/mission`, {
  ...
  const handleEvidence = async (evidence) => {
    ...
    if (token && session?.id) {
      try {
        await fetch(`/api/v1/sessions/${session.id}/evidence`, {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
            Authorization: `Bearer ${token}`
          ...
          body: JSON.stringify({
            ...
            entity_id: mission?.activity_id || 'act-med-101',
            payload: { score: evidence?.score || 1.0, is_correct: isSuccess }
          })
        });
      } catch (err) {
        console.warn('Evidence recording notice:', err);
      }
    }
  };
      ...
      <div className="max-w-5xl mx-auto space-y-8">
        ...
        <header className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-slate-800 pb-6">
          ...
          <div className="flex items-center gap-3">
            <button
              ...
              className="flex items-center gap-2 px-3 py-2 rounded-lg bg-slate-900 border border-slate-700 text-xs font-medium text-slate-300 hover:text-white transition font-mono"
            >
            ...
            <button
              ...
              className="flex items-center gap-2 px-4 py-2 rounded-lg bg-indigo-600 hover:bg-indigo-500 text-xs font-semibold text-white shadow-lg shadow-indigo-600/20 transition font-mono"
            >
        ...
        <AnimatePresence>
          {showExplainability && mission?.explainability && (
            ...
            >
              <div className="flex items-center gap-2 text-xs font-bold uppercase tracking-wider text-indigo-300 font-mono">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 pt-2">
                {Object.entries(mission.explainability.formula_factors || {}).map(([k, v]) => (
                  <div key={k} className="p-2.5 rounded-lg bg-slate-900/80 border border-slate-800 text-xs font-mono">
        ...
        {loading ? (
          ...
          <div className="space-y-6">
            ...
            {feedback && (
              ...
              >
                <div className="flex items-center gap-2">
                  {feedback.success ? <CheckCircle2 className="w-4 h-4 text-emerald-400" /> : <AlertTriangle className="w-4 h-4 text-amber-400" />}
